        ...
        : BlocBuilder<PartyBloc, PartyState>(
            builder: (context, state) {
              ...
              if (state is PartiesLoaded) {
                print(
                    'DEBUG: UI State Loaded. Total in state: ${state.parties.length}');
                final filterType = storeProvider.selectedPartyType;
                print('DEBUG: UI Filter Type: $filterType');

                final parties = state.parties.where((party) {
                  final keep = party.type == filterType;
                  print(
                      'DEBUG: Party ${party.name} (${party.type}) matched? $keep');
                  return keep;
                }).toList();
          ...
          children: [
            ...
            if (party.phone != null && party.phone!.isNotEmpty)
              Flexible(
                // Fix: Allow text to wrap/shrink
                child:
                    Text('📞 ${party.phone}', overflow: TextOverflow.ellipsis),
              ),
          ],
        ),
