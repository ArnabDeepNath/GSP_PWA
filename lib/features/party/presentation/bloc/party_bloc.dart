...
class PartyBloc extends Bloc<PartyEvent, PartyState> {
  ...
  Future<void> _onLoadParties(
    ...
    try {
      ...
      await emit.forEach(
        partiesStream,
        onData: (List<Party> parties) {
          print('DEBUG: PartyBloc received ${parties.length} parties');
          for (var p in parties)
            print('DEBUG: Party in Bloc: ${p.name} - ${p.type}');
          return PartiesLoaded(parties);
        },
        onError: (error, _) {
          print('DEBUG: PartyBloc error: $error');
          return PartyError(error.toString());
        },
      );
    } catch (e) {
