...
class Party {
  ...
  factory Party.fromSnapshot(DocumentSnapshot snapshot) {
    ...
    // Map legacy 'category' field (sale/purchase) to 'type' (buyer/seller)
    PartyType determineType() {
      if (data.containsKey('type')) {
        return data['type'] == 'buyer' ? PartyType.buyer : PartyType.seller;
      } else if (data.containsKey('category')) {
        // 'sale' -> Buyer (Customer), 'purchase' -> Seller (Supplier)
        return data['category'] == 'sale' ? PartyType.buyer : PartyType.seller;
      }
      return PartyType.buyer; // Default
    }

    // Map 'contactNumber' to 'phone'
    String? determinePhone() {
      return data['phone'] ?? data['contactNumber'];
    }
    ...
    return Party(
      ...
      name: data['name'] ?? '',
      type: determineType(),
      gstin: data['gstin'] ?? data['gstNumber'], // Also check gstNumber
      address: data['address'],
      phone: determinePhone(),
      email: data['email'],
      storeId: data['storeId'] ?? '', // This might be missing in old data!
